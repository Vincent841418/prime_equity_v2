                        ...
                        with tab_corp:
                            ...
                            sp200_components = {
                                "Information Technology": [
                                    "AAPL", "ACN", "ADBE", "AMD", "ANET", "APH", "ADI", "AVGO", "CRM", "CSCO", 
                                    "CDNS", "IBM", "INTC", "INTU", "KLAC", "LRCX", "MSFT", "MU", "NOW", "NVDA", 
                                    "ORCL", "PANW", "PLTR", "QCOM", "SNPS", "TXN"
                                ],
                                "Health Care": [
                                    "ABBV", "ABT", "AMGN", "BDX", "BMY", "BSX", "CI", "CVS", "DHR", "ELV", 
                                    "GILD", "HUM", "ISRG", "JNJ", "LLY", "MDT", "MRK", "PFE", "SYK", "TMO", 
                                    "UNH", "VRTX", "ZTS"
                                ],
                                "Consumer Discretionary": [
                                    "ABNB", "AMZN", "BKNG", "CMG", "F", "GM", "HD", "LOW", "MAR", "MCD", 
                                    "NKE", "ORLY", "ORCL", "ROST", "SBUX", "TGT", "TJX", "TSLA", "YUM"
                                ],
                                "Communication Services": [
                                    "CMCSA", "DIS", "GOOG", "GOOGL", "META", "NFLX", "T", "TMUS", "VZ"
                                ],
                                "Industrials": [
                                    "BA", "CAT", "CP", "DE", "ETN", "EMR", "FDX", "GD", "GE", "HON", 
                                ...
                                "Energy": [
                                    "COP", "CVX", "EOG", "MPC", "OXY", "PSX", "SLB", "VLO", "XOM"
                                ],
                                "Real Estate": [
                            ...
                            # Aplanamos el diccionario a una lista simple de tickers (sin duplicados)
                            sp200_tickers = sorted(list(set([ticker for tickers_sector in sp200_components.values() for ticker in tickers_sector])))
                            ...
                            try:
                                ...
                                rf_actual = bono_10a.fast_info.get("lastPrice", 4.25)
                            except:
                                rf_actual = 4.25
                            ...
                            if st.button("🚀 Search for Economic Value opportunities"):
                                if not lista_final:
                                    ...
                                    with st.spinner("Downloading fundamentals & verifying data integrity..."):
                                        ...
                                        if datos_crudos and len(datos_crudos) > 0:
                                            ...
                                            # 🛡️ 1. BLINDAJE INICIAL: Asegurar existencia de columnas clave
                                            columnas_verificar = {
                                            ...
                                            # 🛑 2. FILTRADO GENERALIZADO DE CALIDAD DE DATOS (Múltiples Métricas)
                                            # Evaluamos de forma integral si alguna variable crítica falla
                                            
                                            cond_ebit_invalido = (df_resultados['EBIT ($M)'] <= 0)
                                            cond_mcap_invalido = (df_resultados['Market Cap ($M)'] <= 0)
                                            cond_cap_invertido_invalido = ((df_resultados['Deuda Promedio ($M)'] + df_resultados['Patrimonio Promedio ($M)']) <= 0)
                                            cond_interest_invalido = (df_resultados['Deuda Promedio ($M)'] > 0) & (df_resultados['Interest Expense ($M)'] == 0)
                                            
                                            # Condición global de inconsistencia o datos faltantes en Yahoo
                                            condicion_data_incompleta = (
                                                cond_ebit_invalido | 
                                                cond_mcap_invalido | 
                                                cond_cap_invertido_invalido | 
                                                cond_interest_invalido
                                            )
                                            # 🆕 Diagnóstico: qué variable(s) fallaron por fila
                                            def diagnosticar_exclusion(row):
                                                motivos = []
                                                if row['EBIT ($M)'] <= 0:
                                                    motivos.append("EBIT ($M)")
                                                if row['Market Cap ($M)'] <= 0:
                                                    motivos.append("Market Cap ($M)")
                                                if (row['Deuda Promedio ($M)'] + row['Patrimonio Promedio ($M)']) <= 0:
                                                    motivos.append("Deuda Promedio + Patrimonio Promedio")
                                                if (row['Deuda Promedio ($M)'] > 0) and (row['Interest Expense ($M)'] == 0):
                                                    motivos.append("Interest Expense ($M)")
                                                return motivos
                                            
                                            df_excluidas_data = df_resultados[condicion_data_incompleta].copy()
                                            df_filtrado_valido = df_resultados[~condicion_data_incompleta].copy()

                                            lista_excluidas = df_excluidas_data['Ticker'].tolist()

                                            # 🆕 Ticker -> lista de variables que fallaron
                                            df_excluidas_data['Variables Faltantes'] = df_excluidas_data.apply(diagnosticar_exclusion, axis=1)
                                            dict_motivos_exclusion = dict(zip(df_excluidas_data['Ticker'], df_excluidas_data['Variables Faltantes']))

                                            # 🆕 Todas las variables con problema en este batch (resumen general)
                                            variables_con_problemas = sorted(set(v for lista_v in dict_motivos_exclusion.values() for v in lista_v))
                                            
                                            # ==========================================
                                            # 🟢 CONTROL DE FLUJO: VALIDAR SI QUEDAN DATOS LIMPIOS
                                            # ==========================================
                                            if not df_filtrado_valido.empty:
                                                ...
                                                # ==========================================
                                                # 4. DESPLIEGUE FINAL EN PANTALLA
                                                # ==========================================
                                                st.balloons()
                                                st.success(f"🎯 Complex analysis terminated. Evaluated: {len(df_filtrado_valido)} firms with 100% complete data.")
